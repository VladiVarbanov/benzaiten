...
### Adversarial repair implementation status — 2026-09-15

The nine confirmed findings in the [preserved adversarial review](review/2026-09-15_managed_work_iteration3_adversarial_review.md) have targeted deterministic corrections. These implement the frozen rules above; they do not change authority or introduce a new protocol, manager, lifecycle, or provider.

- Missing evaluation/successor meaning uses bounded grounded same-producer completion, preserving supplied judgments. Conditional representation inconsistencies use bounded representation repair. Normal-planning completion now retains the actual proposal/assessment/synthesis context.
- Trusted numbered inputs retain historical execution evidence and shared step targets. Director chooses relevance; workers receive only selected evidence. Successor reasoning receives both the accepting checkpoint evidence and triggering execution.
- Exhausted structural worker repair creates a failed execution with no successful result. An incompatible ACCEPT is rejected explicitly; Python does not manufacture a replacement semantic decision.
- Charged attempts are saved before dispatch. Immutable call artifacts also hold worker evidence, validated evaluation including `continue_work`, and successor acceptance/publication links. Final TaskExecution artifacts are written once after linkage settles; repeated saves preserve their original bytes/timestamps. Only the passive resume pointer is replaced.
- Worker/evaluation interruptions preserve produced evidence without replay. Accepted successors are durable before the next selection. Reload validates contextual references, certification, checkpoint/trigger links and actual recorded judgments; a `completed` pointer alone cannot establish fulfillment. Root and execution-accepted checkpoints remain distinct, including ACCEPT against `@r1`.
- The launcher and direct controller require an explicit trusted prior planning-call count until automatic planning-trace handoff exists. Reload counts failed/incomplete attempts too; execution transitions remain `highest published revision - 1`.

Validation: 146 focused Managed-Work/bootstrap tests, 72 Normal-planning tests, and 247 full-suite tests passed; Python compilation and `git diff --check` passed. This includes a normally assembled/certified deterministic root with its real seven-call planning count handed to execution and validated reload. The ledger records exact commands and corrected old expectations. No live scenarios, commits, pushes, CURRENT regeneration or TARGET edits were performed.

Limitations remain explicit: returned-guidance incorporation, resumable pre-certification guidance, reciprocal execution and new provider integrations are deferred. Interrupted evidence is reloadable, but no automatic replay/continuation or exactly-once external execution is claimed. Legacy persisted jobs lacking the newly required call/judgment evidence fail validation rather than inventing completion or zero prior consumption; no migration or live revalidation was performed.
